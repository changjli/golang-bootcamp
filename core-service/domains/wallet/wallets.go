...
type WalletRepository interface {
	...
	Create(ctx *gin.Context, userID int) (*entities.Wallet, error)

	// GetByUserID retrieves a user's wallet using their user ID.
	GetByUserID(ctx *gin.Context, userID int) (*entities.Wallet, error)
	...
	UpdateBalance(ctx *gin.Context, userID int, newBalance float64) error
}
...
type WalletUsecase interface {
	...
	GetBalance(ctx *gin.Context, userID int) (*responses.WalletBalanceResponse, error)
}
